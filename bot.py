...
BACKUP_FILE = 'powerhost_backup.pkl'
...
RUN echo '{welcome_message}' > /etc/motd && \\
    ...
    echo 'powerhost-{vps_id}' > /etc/hostname
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        hostname_cmd = f"echo 'powerhost-{vps_id}' > /etc/hostname && hostname powerhost-{vps_id}"
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'powerhost-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "powerhost-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        ...
        try:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    detach=True,
                    privileged=True,
                    hostname=f"powerhost-{self.vps_id}",
                    mem_limit=memory_bytes,
                    cpu_period=100000,
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'powerhost-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
